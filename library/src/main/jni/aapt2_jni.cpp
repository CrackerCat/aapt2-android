...
#include "io_github_rohitverma882_aapt2_Aapt2.h"
 ...
 public:
  ...
  void Log(Level level, aapt::DiagMessageActual& actual_msg) override {
    ...
    if (!mid_) {
      ...
      mid_ = env_->GetMethodID(diagnostics_cls, "onLog", "(ILjava/lang/String;JLjava/lang/String;)V");
    }
...
JNIEXPORT jboolean JNICALL Java_io_github_rohitverma882_aapt2_Aapt2_compile(
  ...
  return aapt::CompileCommand(&diagnostics).Execute(compile_args, &std::cerr) == 0 ? JNI_TRUE : JNI_FALSE;
}

JNIEXPORT jboolean JNICALL Java_io_github_rohitverma882_aapt2_Aapt2_link(JNIEnv* env,
                                                                        jclass aapt_obj,
  ...
  JniDiagnostics diagnostics(env, diagnostics_obj);
  return aapt::LinkCommand(&diagnostics).Execute(link_args, &std::cerr) == 0 ? JNI_TRUE : JNI_FALSE;
}

JNIEXPORT jboolean JNICALL Java_io_github_rohitverma882_aapt2_Aapt2_dump(JNIEnv* env,
                                                                        jclass aapt_obj,
                                                                        jobject arguments_obj,
																		jstring outFile,
                                                                        jobject diagnostics_obj) {
  std::vector<ScopedUtfChars> dump_args_jni = list_to_utfchars(env, arguments_obj);
  std::vector<StringPiece> dump_args = extract_pieces(dump_args_jni);
  ScopedUtfChars name(env, outFile);
  int outFd = open(name.c_str(), O_WRONLY);
  constexpr size_t kStdOutBufferSize = 1024u;
  aapt::io::FileOutputStream fout(outFd, kStdOutBufferSize);
  aapt::text::Printer printer(&fout);
  JniDiagnostics diagnostics(env, diagnostics_obj);
  int rc = aapt::DumpCommand(&printer, &diagnostics).Execute(dump_args, &std::cerr);
  close(outFd);
  return rc == 0 ? JNI_TRUE : JNI_FALSE;
}

JNIEXPORT jboolean JNICALL Java_io_github_rohitverma882_aapt2_Aapt2_convert(JNIEnv* env,
                                                                        jclass aapt_obj,
                                                                        jobject arguments_obj) {
  std::vector<ScopedUtfChars> convert_args_jni = list_to_utfchars(env, arguments_obj);
  std::vector<StringPiece> convert_args = extract_pieces(convert_args_jni);
  return aapt::ConvertCommand().Execute(convert_args, &std::cerr) == 0 ? JNI_TRUE : JNI_FALSE;
}
